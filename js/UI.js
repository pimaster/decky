...
UI = {
	...
	transformCard: function(pos){
		...
		if(current.imageUrl == current.card_faces[0].image_uris.small)
			current.imageUrl = current.card_faces[1].image_uris.small;
		else
			current.imageUrl = current.card_faces[0].image_uris.small;
		Deck.display(true)
	},
